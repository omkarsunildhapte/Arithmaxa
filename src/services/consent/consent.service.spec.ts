...
describe('ConsentService', () => {
  ...
  it('has no consent recorded by default', () => {
    ...
    expect(service.choices()).toEqual({ essential: true, functional: false, aiProcessing: false });
  });
  ...
  it('acceptAll() records consent for every category', () => {
    ...
    expect(service.choices()).toEqual({ essential: true, functional: true, aiProcessing: true });
  ...
  it('save() forces essential to true even if passed false', () => {
    service.save({ essential: false, functional: true, aiProcessing: false });

    expect(service.choices().essential).toBe(true);
  });

  it('save() persists the choices to localStorage', () => {
    service.save({ essential: true, functional: true, aiProcessing: false });
  ...
  it('reads a previously saved consent from localStorage on construction', () => {
    ...
    expect(reloaded.choices()).toEqual({ essential: true, functional: true, aiProcessing: true });
  });
